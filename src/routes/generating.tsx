...
import { getReleaseDir } from "@/lib/release-dir";
...
function GeneratingView() {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        // No real shell to invoke yet (see this project's own CLAUDE.md and
        // README) — this simulates the progress a real `mach bundle` run
        // would report, but the release folder it creates is real.
    ...
    <div className="flex flex-col items-center gap-8 text-center">
      ...
      {status === "error" && (
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-muted-foreground max-w-sm text-sm">{t("generating.errorGeneric")}</p>
          <Button type="button" variant="outline" onClick={() => navigate({ to: "/configure" })}>
            {t("generating.startOver")}
          </Button>
        </div>
